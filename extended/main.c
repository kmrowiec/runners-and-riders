...
int main(int argc, char** argv) {
    ...
    load_cp_file("./data_files/cp_times_3.txt");
    load_cp_file("./data_files/cp_times_4.txt");
    load_cp_file("./data_files/cp_times_5.txt");

//    int i = 0;
//    for(i;i<TOTAL_ENTRANTS;i++){
//        disp_ent_status(&entrants[i]);
//    }
//    
//    update_entrant_position("T 1 3 09:25");
//    update_entrant_position("T 4 3 09:26");
//    update_entrant_position("T 5 3 09:28");
//    update_entrant_position("T 7 3 09:30");
//    update_entrant_position("A 14 3 09:35"); //MC
//    update_entrant_position("D 14 3 09:40"); //MC
    
    
    
    
//    int i = 0;
//    for(i; i<TOTAL_ENTRANTS;i++) display_entrant(&entrants[i]);
//    
    //prompt_for_files();
    ...
    while (n != 0) {
        ...
        printf("\n");
        if(!(n>=0 && n<11)){     //checks if input is correct
        ...
        switch (n) {            
            case 1:             //displaying entrant's status
                ...
                disp_ent_status(find_entrant(cn));
                break;
            case 2:             //showing medical cp history
                printf("Give entrant's number: ");
                int en = -1;
                scanf("%d", &en);
                if(!(en>0 && en<=TOTAL_ENTRANTS)){
                        printf("Incorrect input. Try again.\n");
                        continue;
                }
                display_med_hist(find_entrant(en));
                break;
            case 3:             //show how many not yet started
                printf("%d competitor(s) have not yet started.\n",
                        count_entrants("NS"));
                break;
            case 4:             //show how many are on courses
                printf("%d competitor(s) are out on courses.\n",
                        count_entrants("TR"));
                break;
            case 5:             //show how many finished
                printf("%d competitor(s) have finished.\n",
                        count_entrants("F"));
                break;
            case 6:
                //excluded at medical check
                printf("Excluded ad medical checkpoints:\n");
                list_entrants("EM");
                break;
            case 7:
                //excluded for incorrect route
                printf("Excluded for taking incorrect route:\n");
                list_entrants("EI");
                break;
            case 8:             //manually supply CP data
                ...
                break;
            case 9:             //load checkpoints from file
                printf("Give file name or path to file: ");
                char src[100];
                scanf("%s", &src);
                load_cp_file(src);
                break;
            case 10:             //display results
                display_results();
                break;
        }
    }
...
void print_menu() {
    printf("\n1. Show status of an entrant. \n");
    printf("2. Show medical checkpoints history for an entrant.\n");
    printf("3. How many entrants have not yet started?\n");
    printf("4. How many entrants are out on courses?\n");
    printf("5. How many entrants finished?\n");
    printf("6. List entrants excluded for medical reasons.\n");
    printf("7. List entrants excluded for taking incorrect route.\n");
    printf("8. Supply checkpoint data for an entrant.\n");
    printf("9. Load checkpoints data from file.\n");
    printf("10. View results list.\n");
    printf("0. Quit.\n");
...
void display_results(){
    ...
    for(i;i<count;i++){
        ...
        if(ents[i].number<100) printf(" ");
        for(n; n<49-strlen(ents[i].name); n++) printf(" ");
        if(ents[i].total_time.hours==99){
            //no time displayed if entrant still on track
            //and if excluded, EXCL is displayed
            if(ents[i].status[1]=='E') printf("EXCL\n");
            else printf("-:--\n"); 
